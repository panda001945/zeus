...
void ExecuteBuyOrder()
  {
   double price = SymbolInfoDouble(symbol, SYMBOL_ASK);  // Ask price
   double stop_loss_price = price - stop_loss * _Point;
   double take_profit_price = price + take_profit * _Point;

   // Place the Buy order
   int ticket = OrderSend(symbol, OP_BUY, lot_size, price, slippage, stop_loss_price, take_profit_price, "Buy Order", magic_number, 0, clrGreen);
...
void ExecuteSellOrder()
  {
   double price = SymbolInfoDouble(symbol, SYMBOL_BID);  // Bid price
   double stop_loss_price = price + stop_loss * _Point;
   double take_profit_price = price - take_profit * _Point;

   // Place the Sell order
   int ticket = OrderSend(symbol, OP_SELL, lot_size, price, slippage, stop_loss_price, take_profit_price, "Sell Order", magic_number, 0, clrRed);
